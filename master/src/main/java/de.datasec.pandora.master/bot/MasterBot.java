...
public class MasterBot {
    ...
    public MasterBot(Server server, RoundRobinList<Session> sessions, String startUrl, int urlsPerPacket) {
        ...
        latestUrl = new File("latestUrl.sh");
        if (latestUrl.exists()) {
            try {
                BufferedReader bufferedReader = new BufferedReader(new InputStreamReader(new FileInputStream(latestUrl.getName()), Charset.forName("UTF-8")));
    ...
    private void addUrl(String url) {
        if (!cassandraManager.contains(tableName, column, column, url)) {
            ...
            if (urlsToVisit.size() % 100 == 0) {
                ...
                try {
                    BufferedWriter bufferedWriter = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(latestUrl)));
